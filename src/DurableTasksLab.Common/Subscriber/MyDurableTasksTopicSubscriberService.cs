    ...
    private async Task ProcessMessageAsync(ProcessMessageEventArgs args, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (receivedMessage?.Body != null)
            {
                ...
                Assumes.NotNull(sender);
                if(receivedCloudEvent != null){
                    await this.durableTasksMessageHandler.HandleMessage(receivedCloudEvent, sender);
                }
        ...
        {
            await args.DeadLetterMessageAsync(receivedMessage, "Exception Thrown", e.Message);
        }
    }
